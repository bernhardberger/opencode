...
const wellKnown = (input: {
  ...
  const authWellKnown = Layer.mock(AuthWellKnown.Service)({
    all: () =>
      Effect.succeed({
        [authUrl]: new AuthWellKnown.Entry({ key: "TEST_TOKEN", token: "test-token" }),
      }),
    configs: () =>
      Effect.sync(() => {
        ...
        const metadata = document as { config?: unknown; remote_config?: { url?: unknown; headers?: unknown } }
        if (metadata.config !== undefined && metadata.config !== null) {
          docs.push({ url: authUrl, source, dir: path.dirname(source), content: metadata.config })
        }
        if (metadata.remote_config && typeof metadata.remote_config.url === "string") {
          const remote = metadata.remote_config.url.replaceAll("{env:TEST_TOKEN}", "test-token")
          const headers =
            metadata.remote_config.headers && typeof metadata.remote_config.headers === "object"
              ? Object.fromEntries(
                  Object.entries(metadata.remote_config.headers).flatMap(([key, value]) =>
                    typeof value === "string" ? [[key, value.replaceAll("{env:TEST_TOKEN}", "test-token")]] : [],
                  ),
                )
              : undefined
          seen.remote = remote
          seen.authorization = typeof headers?.Authorization === "string" ? headers.Authorization : undefined
          docs.push({
            url: remote,
            source: remote,
            dir: path.dirname(remote),
            content:
              input.remote && typeof input.remote === "object" && input.remote !== null && "config" in input.remote
